...
@app.route('/')
def index():
    # First landing page should be the public hospital home (pics only)
...
def register():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            flash('Already have an account? Please login.', 'danger')
...
@app.route('/delete_other/<int:id>', methods=['POST'])
def delete_other(id):
    other = Other.query.get_or_404(id)
